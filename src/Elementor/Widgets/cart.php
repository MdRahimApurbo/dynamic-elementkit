...
class DEK_Cart_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return __('Cart Icon', 'dynamic-elementkit');
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'cart_text',
            [
                'label' => __('Cart Text', 'dynamic-elementkit'),
        ...
        $this->add_control(
            'show_count',
            [
                'label' => __('Show Count', 'dynamic-elementkit'),
        ...
        $this->add_control('drawer_title', [
            'label' => __('Cart Drawer Title', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'default' => __('আপনার কার্ট', 'dynamic-elementkit'),
            'separator' => 'before',
        ]);

        $this->add_control('empty_text', [
            'label' => __('Empty Cart Message', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'default' => __('আপনার শপিং কার্টটি খালি।', 'dynamic-elementkit'),
        ]);

        $this->add_control('continue_text', [
            'label' => __('Continue Shopping Text', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'default' => __('কেনাকাটা চালিয়ে যান', 'dynamic-elementkit'),
        ]);

        $this->add_control('subtotal_text', [
            'label' => __('Subtotal Label', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'default' => __('মোট পণ্যমূল্য', 'dynamic-elementkit'),
        ]);

        $this->add_control('shipping_text', [
            'label' => __('Shipping Message', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXTAREA,
            'default' => __('চেকআউটের সময় ডেলিভারি চার্জ হিসাব করা হবে।', 'dynamic-elementkit'),
        ]);

        $this->add_control('checkout_text', [
            'label' => __('Checkout Button Text', 'dynamic-elementkit'),
            'type' => \Elementor\Controls_Manager::TEXT,
            'default' => __('অর্ডার সম্পন্ন করুন', 'dynamic-elementkit'),
        ]);
        ...
        $this->start_controls_section(
            'icon_style_section',
            [
                'label' => __('Cart Icon', 'dynamic-elementkit'),
        ...
        $this->add_control(
            ...
            [
                ...
                'label' => __('Icon Size', 'dynamic-elementkit'),
    ...
    protected function render() {
        ...
        $drawer_title = !empty($settings['drawer_title']) ? $settings['drawer_title'] : __('আপনার কার্ট', 'dynamic-elementkit');
        $empty_text = !empty($settings['empty_text']) ? $settings['empty_text'] : __('আপনার শপিং কার্টটি খালি।', 'dynamic-elementkit');
        $continue_text = !empty($settings['continue_text']) ? $settings['continue_text'] : __('কেনাকাটা চালিয়ে যান', 'dynamic-elementkit');
        $subtotal_text = !empty($settings['subtotal_text']) ? $settings['subtotal_text'] : __('মোট পণ্যমূল্য', 'dynamic-elementkit');
        $shipping_text = !empty($settings['shipping_text']) ? $settings['shipping_text'] : __('চেকআউটের সময় ডেলিভারি চার্জ হিসাব করা হবে।', 'dynamic-elementkit');
        $checkout_text = !empty($settings['checkout_text']) ? $settings['checkout_text'] : __('অর্ডার সম্পন্ন করুন', 'dynamic-elementkit');
        $count = class_exists('WooCommerce') && WC()->cart ? WC()->cart->get_cart_contents_count() : 0;

        $nonce = wp_create_nonce('dek_toggle_active');
        if (!class_exists('WooCommerce') || !WC()->cart || WC()->cart->is_empty()) {
            ...
            foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                ...
                $items_html .= '<span class="wpb-cart-item-qty">' . esc_html__('পরিমাণ:', 'dynamic-elementkit') . ' ' . esc_html($cart_item['quantity']) . '</span>';
                $items_html .= '<span class="wpb-cart-item-price">' . wp_kses_post($price) . '</span>';
                $items_html .= '</div>';
                $items_html .= '<button type="button" class="wpb-cart-remove" data-key="' . esc_attr($cart_item_key) . '" aria-label="' . esc_attr__('কার্ট থেকে সরান', 'dynamic-elementkit') . '"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"></path></svg></button>';
                $items_html .= '</div>';
            }
        }
        ?>
        <div class="wpb-cart" style="display:inline-block;position:relative;"
            ...
            <div class="wpb-cart-panel">
                <div class="wpb-cart-panel-header">
                    ...
                    <button type="button" class="wpb-cart-close" aria-label="<?php esc_attr_e('কার্ট বন্ধ করুন', 'dynamic-elementkit'); ?>">
        ...
        (function() {
            ...
            carts.forEach(function(wrapper) {
                ...
                var ajaxUrl = typeof dekAdmin !== 'undefined' && dekAdmin.ajaxUrl ? dekAdmin.ajaxUrl : '';
                ...
                function refreshCart() {
                    ...
                    xhr.send('action=dek_get_cart_fragments&_wpnonce=' + encodeURIComponent(nonce));
                ...
                function bindRemoveButtons() {
                    ...
                    btns.forEach(function(btn) {
                        btn.addEventListener('click', function(e) {
                            ...
                            xhr.send('action=dek_cart_remove_item&cart_item_key=' + encodeURIComponent(key) + '&_wpnonce=' + encodeURIComponent(nonce));
                        });
                    });
                }
        ...
        <div class="wpb-cart" style="display:inline-block;position:relative;">
            ...
            <div class="wpb-cart-panel" style="position:fixed;top:0;right:0;width:min(420px,100vw);max-width:100%;height:100%;background:#ffffff;z-index:9999;transform:translateX(100%) !important;transition:transform 0.3s ease;opacity:0;visibility:hidden;pointer-events:none;display:flex;flex-direction:column;">
                <div class="wpb-cart-panel-header" style="padding:16px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <button class="wpb-cart-close" type="button" aria-label="<?php esc_attr_e('কার্ট বন্ধ করুন', 'dynamic-elementkit'); ?>"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18"></path></svg></button>
                </div>
